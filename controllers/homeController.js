...
const inscrptionView = (req, res, next) => {
    if(req.query.token){
        return  res.render('inscription', {path:req.path, detail: false,countries:Object.values(countries),filiere:my_filieres,user:res.locals.user,message:'',modifier:false})
   
    }
    res.render('inscription', {path:req.path, detail: false,countries:Object.values(countries),filiere:my_filieres,user:null,message:'',modifier:false})
}
